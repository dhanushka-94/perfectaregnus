...
<form action="{{ route('admin.products.update') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="grid lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div class="space-y-4 sm:space-y-6">
            ...
            <div class="admin-card p-4 sm:p-6 space-y-4">
                <h2 class="font-display text-lg font-semibold text-ink">Product Image</h2>
                @if($product->image_url)
                    <img src="{{ $product->image_url }}" alt="{{ $product->name }}" class="w-full object-contain rounded-sm bg-cream max-h-48">
                @endif
                <div>
                    <label for="image" class="block text-sm font-medium text-ink mb-1.5">Upload product image</label>
                    ...
                    <p class="text-xs text-muted mt-1">JPG, PNG, WebP or GIF. Max 5MB. Used across the storefront.</p>
                </div>
            </div>
        </div>
    </div>
